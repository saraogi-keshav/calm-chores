  ...
  const handleToggleComplete = async (
    ...
    try {
      ...
      await updateDoc(taskRef, {
        ...
        overdueCompletion: !currentStatus
        ...
        if (taskData.alwaysRepeat) {
          ...
          const newDueDate = new Date(
            currentDueDate.setDate(
              currentDueDate.getDate() + taskData.repeatDays
            )
          );
    ...
    switch (selectedTab) {
      case "mine":
        filtered = tasks.filter(
          (task) => task.assignedTo === user?.uid && !task.completed
        );
        break;
      case "completed":
    ...
    <SafeAreaView className="flex-1 bg-gray-100 dark:bg-gray-900">
      ...
      <View className="flex-1 p-4">
        <FlatList
          ...
          renderItem={({ item }) => {
            const assignedUser = house?.users.find(
              (u) => u.id === item.assignedTo
            );
            const isUserOnVacation = assignedUser?.vacationMode;

            return (
              <View className={`bg-white dark:bg-gray-800 p-4 rounded-md mb-2`}>
                {isUserOnVacation && (
                  <View className="bg-blue-100 dark:bg-blue-700 px-2 py-1 rounded-full mb-3 self-start">
                    <Text className="text-gray-600 dark:text-gray-400 text-sm italic">
                      Assigned user is on vacation!
                    </Text>
                  </View>
                )}
                ...
                >
                  ...
                  {item.completed && item.completedBy && (
                    <View
                      className={`bg-green-100 dark:bg-green-900 px-2 py-1 rounded-full opacity-60`}
                    >
                      <Text className="text-xs text-green-700 dark:text-green-300">
                        Completed By:{" "}
                        {item.completedBy === user?.uid
                          ? user?.displayName || user?.email || "Me"
                          : house?.users.find((u) => u.id === item.completedBy)
                              ?.displayName ||
                            house?.users.find((u) => u.id === item.completedBy)
                              ?.email ||
                            "Unknown User"}
                  ...
                  >
                    ...
                    >
                      ...
                      {item.assignedTo === user?.uid
                        ? user?.displayName || user?.email || "Me"
                        : item.assignedTo
                        ? house?.users.find((u) => u.id === item.assignedTo)
                            ?.displayName ||
                          house?.users.find((u) => u.id === item.assignedTo)
                            ?.email ||
                          "Unknown User"
      ...
      if (task.assignedTo) {
        if (!task.completed) {
          assignments[task.assignedTo] =
            (assignments[task.assignedTo] || 0) + 1;
          ...
          if (daysSinceCompletion < 7) {
            assignments[task.assignedTo] =
              (assignments[task.assignedTo] || 0) + 0.5;
          }
        }
      }
    });
...
const getNextUserInRotation = async (
  houseId: string,
  houseUsers: any[],
  repeatTaskId: string | null,
  ...
  try {
    ...
    rotationSnapshot.forEach((doc) => {
      const task = doc.data();
      if (task.assignedTo) {
        rotationCounts[task.assignedTo] =
          (rotationCounts[task.assignedTo] || 0) + 1;
      }
    });
    ...
    const eligibleUsers = Object.entries(rotationCounts)
      .filter(
        ([userId, count]) =>
          count === minAssignments && userId !== currentAssignee
      )
      .map(([userId]) => userId);
    ...
    if (eligibleUsers.length === 0) {
      ...
      return anyoneExceptCurrent[
        Math.floor(Math.random() * anyoneExceptCurrent.length)
      ];
    }
